...
<body>
    ...
    Java 코드를 쉽고 표기법을 간단히 태그화하여 표준으로 제공하는 것.

    (if, for, 배열/컬랙션 길이, 문자열 치환, 숫자 데이터 형식 변경, 
    데이터 파싱, scope 변수 선언 등 )
   
    </pre>

     <h3>JSLT 라이브러리 등록 방법 </h3>
     ...
     <pre>
     	JSP 파일 최상단에 추가하고자 하는 JSTL taglib 를 추가  
     </pre>
     	...
     	6) varStatus : 현재 반복 상태와 관련된 정보를 제공하는 변수 
     		...
     		* varStatus에서 제공되는 값
     		- current : 현재 반복 횟수(현재 var 값) 
     					또는 현재 반복 접근중인 객체(컬랙션/배열 요소) 	 	
     		- index : 현재 몇번째 인덱스값 반환 (0부터 시작)
